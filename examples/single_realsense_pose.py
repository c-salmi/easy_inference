...
for (rgb_frame, depth_frame, pointcloud) in cam1:
    ...
    rgb_frame = pad_frames(rgb_frame, width=network_input_dim[0], height=network_input_dim[1], type='rgb')
    depth_frame = pad_frames(depth_frame, width=network_input_dim[0], height=network_input_dim[1], type='depth')
    ...
    persons = [Skeleton(*output[0:4], class_id=output[5], confidence=output[4], kpts=output[6:]) for output in outputs]
