...
cam5 = Realsense(
    width=width, height=height, depth=True, pointcloud=True, device="215122255929"
)
cam1 = Realsense(
    width=width, height=height, depth=True, pointcloud=True, device="215122255869"
)
providers = [cam5, cam1]
...
for frames in combine(*providers):
    ...
    rgb_frames = pad_frames(
        rgb_frames, width=network_input_dim[0], height=network_input_dim[1]
    )
    depth_frames = pad_frames(
        depth_frames, width=network_input_dim[0], height=network_input_dim[1]
    )
    ...
    persons3d = [
        person.to3d(
            depth_frames[person.batch_id], providers[person.batch_id]._depth_intr
        )
    ...
    if SHOW:
        ...
        f_rgb = np.ascontiguousarray(
            rgb_frames[batch_id].transpose(1, 2, 0).astype(np.uint8)
        )
