...
class Skeleton(BoundingBox):
    ...
    def __init__(
        self, x0, y0, x1, y1, class_id, confidence, kpts, batch_id=0, frame_id=None
    ) -> None:
        ...
        self.kpts = [
            (
                self._discretize(kpts[3 * i]),
                self._discretize(kpts[3 * i + 1]),
                kpts[3 * i + 2],
            )
    ...
    def to3d(self, depth_frame, intrinsics, depth_scale=0.001) -> Skeleton3d:
        ...
        x, y, z = rs.rs2_deproject_pixel_to_point(
            intrinsics, self.center, pose_refined_depth
        )
        box3d.x = x
        box3d.y = y
        box3d.z = z
